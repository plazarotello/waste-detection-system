...
sha = "Unknown"
package_name = "custom_torchvision"
...
def write_version_file():
    version_path = os.path.join(cwd, "custom_torchvision", "version.py")
    with open(version_path, "w") as f:
        f.write(f"__version__ = '{version}'\n")
        f.write(f"git_version = {repr(sha)}\n")
        f.write("from custom_torchvision.extension import _check_cuda_version\n")
...
def find_library(name, vision_include):
    ...
    package_path = [os.path.join(this_dir, "custom_torchvision")]
...
def get_extensions():
    this_dir = os.path.dirname(os.path.abspath(__file__))
    extensions_dir = os.path.join(this_dir, "custom_torchvision", "csrc")
    ...
    if is_rocm_pytorch:
        ...
        hipify_python.hipify(
            ...
            includes="custom_torchvision/csrc/ops/cuda/*",
            show_detailed=True,
            is_pytorch_extension=True,
        )
        source_cuda = glob.glob(os.path.join(extensions_dir, "ops", "hip", "*.hip"))
        # Copy over additional files
        for file in glob.glob(r"custom_torchvision/csrc/ops/cuda/*.h"):
            shutil.copy(file, "custom_torchvision/csrc/ops/hip")
    else:
    ...
    if compile_cpp_tests:
        ...
        models_dir = os.path.join(this_dir, "custom_torchvision", "csrc", "models")
    ...
    # ------------------- custom_torchvision extra extensions ------------------------
    ...
    if png_found:
        if libpng is not None:
            ...
            if png_version >= parse_version("1.6.0"):
                print("Building custom_torchvision with PNG image support")
    ...
    if jpeg_found:
        print("Building custom_torchvision with JPEG image support")
    ...
    if nvjpeg_found:
        print("Building custom_torchvision with NVJPEG image support")
    ...
    # FIXME: Building custom_torchvision with ffmpeg on MacOS or with Python 3.9
    ...
    if has_ffmpeg:
        ...
        # custom_torchvision base decoder + video reader
        video_reader_src_dir = os.path.join(this_dir, "custom_torchvision", "csrc", "io", "video_reader")
        video_reader_src = glob.glob(os.path.join(video_reader_src_dir, "*.cpp"))
        base_decoder_src_dir = os.path.join(this_dir, "custom_torchvision", "csrc", "io", "decoder")
        base_decoder_src = glob.glob(os.path.join(base_decoder_src_dir, "*.cpp"))
        # custom_torchvision video API
        videoapi_src_dir = os.path.join(this_dir, "custom_torchvision", "csrc", "io", "video")
        ...
        ext_modules.append(
            CppExtension(
                "custom_torchvision.video_reader",
                combined_src,
                include_dirs=[
